...
SceneSnapshot EditorRuntimeHost::getSceneSnapshot() const
{
    ...
    for (const auto& obj : charLayer.getObjects())
    {
        if (obj->isVisible())
        {
            ...
            {
                snapshot.characterExpressions.emplace_back(
...
void EditorRuntimeHost::setBreakpointEnabled(const std::string& scriptPath, u32 line, bool enabled)
{
    for (auto& bp : m_breakpoints)
    {
...
Result<void> EditorRuntimeHost::compileProject()
{
    try
    {
        ...
        if (allScripts.empty())
        {
            return Result<void>::error("No script files found in: " + m_project.scriptsPath);
        }
...
Result<void> EditorRuntimeHost::initializeRuntime()
{
    ...
    m_scriptRuntime->setEventCallback(
        [this](const scripting::ScriptEvent& event)
        {
