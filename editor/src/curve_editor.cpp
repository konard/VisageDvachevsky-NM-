...
f32 AnimationCurve::evaluate(f32 t) const
{
    ...
    if (segIndex < 0) return m_points[0].value;
    ...
    if (segmentLength < 0.0001f) return p0.value;
...
AnimationCurve AnimationCurve::createEaseInBounce()
{
    ...
    CurvePoint p0;
    p0.time = 0.0f; p0.value = 0.0f;
    curve.m_points.push_back(p0);

    CurvePoint p1;
    p1.time = 0.09f; p1.value = 0.03f;
    curve.m_points.push_back(p1);

    CurvePoint p2;
    p2.time = 0.27f; p2.value = 0.06f;
    curve.m_points.push_back(p2);

    CurvePoint p3;
    p3.time = 0.55f; p3.value = 0.23f;
    curve.m_points.push_back(p3);

    CurvePoint p4;
    p4.time = 1.0f; p4.value = 1.0f;
...
AnimationCurve AnimationCurve::createEaseOutBounce()
{
    ...
    CurvePoint p1;
    p1.time = 0.36f; p1.value = 0.77f;
    curve.m_points.push_back(p1);

    CurvePoint p2;
    p2.time = 0.73f; p2.value = 0.94f;
    curve.m_points.push_back(p2);

    CurvePoint p3;
    p3.time = 0.91f; p3.value = 0.97f;
    curve.m_points.push_back(p3);
...
AnimationCurve AnimationCurve::createEaseInOutBounce()
{
    ...
    CurvePoint p1;
    p1.time = 0.15f; p1.value = 0.03f;
    curve.m_points.push_back(p1);

    CurvePoint p2;
    p2.time = 0.5f; p2.value = 0.5f;
    curve.m_points.push_back(p2);

    CurvePoint p3;
    p3.time = 0.85f; p3.value = 0.97f;
    curve.m_points.push_back(p3);
...
AnimationCurve AnimationCurve::createEaseInElastic()
{
    ...
    CurvePoint p1;
    p1.time = 0.4f; p1.value = -0.01f;
    curve.m_points.push_back(p1);

    CurvePoint p2;
    p2.time = 0.7f; p2.value = 0.02f;
    curve.m_points.push_back(p2);

    CurvePoint p3;
    p3.time = 0.85f; p3.value = -0.06f;
    curve.m_points.push_back(p3);
...
AnimationCurve AnimationCurve::createEaseOutElastic()
{
    ...
    CurvePoint p1;
    p1.time = 0.15f; p1.value = 1.06f;
    curve.m_points.push_back(p1);

    CurvePoint p2;
    p2.time = 0.3f; p2.value = 0.98f;
    curve.m_points.push_back(p2);

    CurvePoint p3;
    p3.time = 0.6f; p3.value = 1.01f;
    curve.m_points.push_back(p3);
...
AnimationCurve AnimationCurve::createEaseInOutElastic()
{
    ...
    CurvePoint p1;
    p1.time = 0.35f; p1.value = 0.0f;
    curve.m_points.push_back(p1);

    CurvePoint p2;
    p2.time = 0.45f; p2.value = -0.03f;
    curve.m_points.push_back(p2);

    CurvePoint p3;
    p3.time = 0.5f; p3.value = 0.5f;
    curve.m_points.push_back(p3);

    CurvePoint p4;
    p4.time = 0.55f; p4.value = 1.03f;
    curve.m_points.push_back(p4);

    CurvePoint p5;
    p5.time = 0.65f; p5.value = 1.0f;
    curve.m_points.push_back(p5);

    CurvePoint p6;
    p6.time = 1.0f; p6.value = 1.0f;
...
bool AnimationCurve::isValid() const
{
    ...
    {
        if (m_points[i].time < m_points[i - 1].time) return false;
    }

    // Check range
    if (m_points.front().time < 0.0f || m_points.back().time > 1.0f) return false;
...
f32 AnimationCurve::evaluateBezierSegment(const CurvePoint& p0, const CurvePoint& p1, f32 localT) const
{
...
void CurveLibrary::removeCurve(const std::string& id)
{
    m_curves.erase(id);
}

void CurveLibrary::updateCurve(const std::string& id, const AnimationCurve& curve)
{
...
void CurveLibrary::setCurveCategory(const std::string& curveId, const std::string& category)
{
    m_curveCategories[curveId] = category;
}

std::vector<std::string> CurveLibrary::getCurvesInCategory(const std::string& category) const
{
...
void CurveLibrary::loadPresets()
{
    ...
    auto addPreset = [this](const AnimationCurve& curve, const std::string& category) {
        m_curves[curve.getId()] = curve;
