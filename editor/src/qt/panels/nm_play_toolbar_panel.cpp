...
void NMPlayToolbarPanel::onInitialize() {
    ...
    connect(&controller, &NMPlayModeController::playModeChanged,
            this, &NMPlayToolbarPanel::onPlayModeChanged);
    connect(&controller, &NMPlayModeController::currentNodeChanged,
            this, &NMPlayToolbarPanel::onCurrentNodeChanged);
    connect(&controller, &NMPlayModeController::breakpointHit,
            this, &NMPlayToolbarPanel::onBreakpointHit);
...
void NMPlayToolbarPanel::updateStatusLabel() {
    ...
    m_statusLabel->setStyleSheet(QString("QLabel { color: %1; padding: 4px 8px; font-weight: bold; }").arg(color));
}
...
void NMPlayToolbarPanel::onBreakpointHit(const QString& nodeId) {
    ...
    m_statusLabel->setStyleSheet("QLabel { color: #f44336; padding: 4px 8px; font-weight: bold; }");  // Red
}
