...
NMConsoleOutput::NMConsoleOutput(QWidget* parent)
...
void NMConsoleOutput::appendLog(const LogEntry& entry)
{
    ...
    switch (entry.level)
    {
        case LogLevel::Debug:   visible = m_showDebug; break;
        case LogLevel::Info:    visible = m_showInfo; break;
        case LogLevel::Warning: visible = m_showWarning; break;
        case LogLevel::Error:   visible = m_showError; break;
    }
...
{
    if (m_output) m_output->setShowDebug(checked);
}

void NMConsolePanel::onToggleInfo(bool checked)
{
    if (m_output) m_output->setShowInfo(checked);
}

void NMConsolePanel::onToggleWarning(bool checked)
{
    if (m_output) m_output->setShowWarning(checked);
}

void NMConsolePanel::onToggleError(bool checked)
{
    if (m_output) m_output->setShowError(checked);
}

void NMConsolePanel::onToggleAutoScroll(bool checked)
{
    if (m_output) m_output->setAutoScroll(checked);
}
