    ...
    , m_objectType(type)
{
    ...
    switch (type)
    {
        ...
        case NMSceneObjectType::Character:
            ...
            painter.drawText(pixmap.rect(), Qt::AlignCenter | Qt::AlignBottom, "Character");
            break;
...
void NMSceneObject::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    // Draw the pixmap
...
void NMTransformGizmo::createRotateGizmo()
{
    ...
    // Outer circle
    auto* circle = new QGraphicsEllipseItem(-radius, -radius, radius * 2, radius * 2, this);
...
void NMTransformGizmo::createScaleGizmo()
{
    ...
    {
        auto* handle = new QGraphicsRectItem(corner.x() - 6, corner.y() - 6, 12, 12, this);
        handle->setBrush(palette.accentPrimary);
        handle->setPen(Qt::NoPen);
        addToGroup(handle);
    }
...
NMSceneInfoOverlay::NMSceneInfoOverlay(QWidget* parent)
    : QWidget(parent)
{
    ...
    m_cursorLabel->setStyleSheet(QString(
        "QLabel {"
    ...
    m_objectLabel->setStyleSheet(QString(
        "QLabel {"
...
void NMSceneViewPanel::addDemoObjects()
{
    ...
    // Add background
    auto* background = new NMSceneObject("bg_main", NMSceneObjectType::Background);
    background->setName("Main Background");
    background->setPos(-100, -150);
    m_scene->addSceneObject(background);

    // Add character 1
    auto* character1 = new NMSceneObject("char_protagonist", NMSceneObjectType::Character);
    character1->setName("Protagonist");
    character1->setPos(-250, -100);
    m_scene->addSceneObject(character1);

    // Add character 2
    auto* character2 = new NMSceneObject("char_companion", NMSceneObjectType::Character);
...
void NMSceneViewPanel::setupToolBar()
{
    ...
    gizmoGroup->addButton(m_toolBar->widgetForAction(actionMove) ?
        qobject_cast<QAbstractButton*>(m_toolBar->widgetForAction(actionMove)) : nullptr);
...
void NMSceneViewPanel::setupContent()
{
    ...
    // Connect signals
    connect(m_view, &NMSceneGraphicsView::cursorPositionChanged,
            this, &NMSceneViewPanel::onCursorPositionChanged);
    connect(m_scene, &NMSceneGraphicsScene::objectSelected,
            this, &NMSceneViewPanel::onSceneObjectSelected);
    connect(m_scene, &NMSceneGraphicsScene::objectPositionChanged,
            this, &NMSceneViewPanel::onObjectPositionChanged);
}
