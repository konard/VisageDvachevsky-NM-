...
void NMTimelinePanel::setupToolbar()
{
    ...
    connect(m_frameSpinBox, QOverload<int>::of(&QSpinBox::valueChanged),
            this, &NMTimelinePanel::setCurrentFrame);
...
void NMTimelinePanel::setupTrackView()
{
    ...
    // Create playhead
    m_playheadItem = m_timelineScene->addLine(
        TRACK_HEADER_WIDTH, 0,
        TRACK_HEADER_WIDTH, 1000,
...
{
    if (!m_tracks.contains(trackName)) return;
...
void NMTimelinePanel::renderTracks()
{
    ...
    for (int frame = 0; frame <= m_totalFrames; frame += 10)
    {
        ...
        {
            QGraphicsTextItem* label = m_timelineScene->addText(QString::number(frame));
    ...
    for (auto it = m_tracks.begin(); it != m_tracks.end(); ++it, ++trackIndex)
    {
        ...
        for (const Keyframe& kf : track->keyframes)
        {
            ...
            QGraphicsEllipseItem* kfItem = m_timelineScene->addEllipse(
                ...
                QPen(track->color.lighter(150), 2),
                QBrush(track->color));
            kfItem->setZValue(10);
        }
    ...
    // Update scene rect
    m_timelineScene->setSceneRect(0, -30,
                                  frameToX(m_totalFrames) + 100,
                                  y + TIMELINE_MARGIN);
