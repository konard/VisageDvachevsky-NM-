...
Result<void> VoiceManager::loadProject(const std::string& projectPath)
{
    ...
    m_voiceAssetsPath = projectPath + "/assets/voice";
...
Result<void> VoiceManager::refreshDialogueLines()
{
    ...
    std::string scriptsPath = m_projectPath + "/scripts";
...
std::vector<const DialogueLine*> VoiceManager::getFilteredLines(const VoiceLineFilter& filter) const
{
...
std::vector<const DialogueLine*> VoiceManager::getLinesForCharacter(const std::string& characterId) const
{
    std::vector<const DialogueLine*> result;
    for (const auto& line : m_dialogueLines)
    {
...
{
    auto lineIt = m_lineIdToIndex.find(lineId);
    if (lineIt == m_lineIdToIndex.end()) return;
...
Result<u32> VoiceManager::autoMapVoiceFiles()
{
    ...
    for (const auto& voiceFile : m_voiceFiles)
    {
        ...
        if (matched)
        {
            ...
            for (auto& line : m_dialogueLines)
            {
                if (line.status != VoiceBindingStatus::Unbound) continue;
...
void VoiceManager::previewVoice(const std::string& lineId)
{
    ...
    if (!line || line->voiceFile.empty()) return;
...
VoiceCoverageStats VoiceManager::getCoverageStats() const
{
    ...
    {
        stats.coveragePercent = (static_cast<f32>(stats.boundLines) / static_cast<f32>(stats.totalLines)) * 100.0f;
    }
...
Result<void> VoiceManager::exportVoiceTable(const std::string& outputPath, VoiceTableFormat format) const
{
...
Result<void> VoiceManager::exportUnboundLines(const std::string& outputPath, VoiceTableFormat format) const
{
...
Result<void> VoiceManager::exportCSV(const std::string& path, const std::vector<const DialogueLine*>& lines) const
{
    ...
    for (const auto* line : lines)
    {
        file << "\"" << line->id << "\","
             ...
             << "\"" << line->text << "\","
             << "\"" << line->voiceFile << "\","
             << static_cast<int>(line->status) << "\n";
    }

    return {};
}

Result<void> VoiceManager::exportJSON(const std::string& path, const std::vector<const DialogueLine*>& lines) const
{
    std::ofstream file(path);
    if (!file.is_open())
    {
...
Result<void> VoiceManager::exportTSV(const std::string& path, const std::vector<const DialogueLine*>& lines) const
{
    std::ofstream file(path);
    if (!file.is_open())
    {
