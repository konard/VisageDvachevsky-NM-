...
public:
     ...
     */
    [[nodiscard]] std::vector<ScriptReference> getReferencesFromScene(const std::string& sceneId) const;
     ...
     */
    [[nodiscard]] std::vector<std::string> getScenesReachableFrom(const std::string& sourceSceneId) const;

    /**
     * @brief Get shortest path between two scenes
     */
    [[nodiscard]] std::vector<std::string> getShortestPath(const std::string& from, const std::string& to) const;
...
public:
     ...
     */
    void renderCallGraphOverlay(renderer::IRenderer* renderer,
