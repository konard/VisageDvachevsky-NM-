...
struct Color
{
    ...
    [[nodiscard]] u32 toHex(bool includeAlpha = false) const
    {
        ...
        if (includeAlpha)
        {
            return (static_cast<u32>(ri) << 24) |
                   (static_cast<u32>(gi) << 16) |
                   (static_cast<u32>(bi) << 8) |
                   static_cast<u32>(ai);
        }
        return (static_cast<u32>(ri) << 16) |
...
public:
    ...
    [[nodiscard]] const PropertyMeta& getMeta() const override
    {
        return m_meta;
    }
 ...
 */
#define NM_BEGIN_TYPE(TypeName) \
    NovelMind::TypeInfoBuilder<TypeName>(#TypeName)
