...
public:
    ...
    [[nodiscard]] Result<PackVerificationReport> verifyHeader(
        const u8* data, usize size);

    [[nodiscard]] Result<PackVerificationReport> verifyResourceTable(
        const u8* data, usize size, u64 tableOffset, u32 resourceCount);

    [[nodiscard]] Result<PackVerificationReport> verifyResource(
        const u8* data, usize size, u64 offset, usize resourceSize, u32 expectedChecksum);

    [[nodiscard]] Result<PackVerificationReport> verifyPackSignature(
...
public:
    ...
    [[nodiscard]] Result<std::vector<u8>> decrypt(
