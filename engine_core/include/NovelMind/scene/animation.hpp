...
{
public:
    PositionTween(f32* targetX, f32* targetY,
                  f32 fromX, f32 fromY,
                  f32 toX, f32 toY,
    ...
    {
        Tween::start();
        if (m_targetX) *m_targetX = m_fromX;
        if (m_targetY) *m_targetY = m_fromY;
    }

protected:
    void applyProgress(f32 progress) override
    {
        if (m_targetX) *m_targetX = m_fromX + (m_toX - m_fromX) * progress;
        if (m_targetY) *m_targetY = m_fromY + (m_toY - m_fromY) * progress;
    }
...
{
public:
    ColorTween(renderer::Color* target,
               const renderer::Color& from,
               const renderer::Color& to,
               f32 duration, EaseType easing = EaseType::Linear)
        : Tween(duration, easing)
        , m_target(target)
        , m_from(from)
