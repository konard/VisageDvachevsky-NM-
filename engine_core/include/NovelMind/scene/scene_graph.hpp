...
public:
    ...
    [[nodiscard]] const std::vector<std::unique_ptr<SceneObjectBase>>& getChildren() const { return m_children; }
...
protected:
    ...
    void notifyPropertyChanged(const std::string& property, const std::string& oldValue, const std::string& newValue);
...
public:
    ...
    [[nodiscard]] const std::vector<std::unique_ptr<SceneObjectBase>>& getObjects() const { return m_objects; }
...
private:
    ...
    // Quick lookup by ID
    std::unordered_map<std::string, SceneObjectBase*> m_objectLookup;
};
