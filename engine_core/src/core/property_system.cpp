...
TypeInfo::getPropertiesByCategory() const
{
    ...
    std::sort(result.begin(), result.end(),
        ...
        {
            if (a.first == "General") return true;
            if (b.first == "General") return false;
...
PropertyValue fromString(PropertyType type, const std::string& str)
{
    switch (type)
    {
        ...
        case PropertyType::Int:
            try { return static_cast<i32>(std::stoi(str)); }
            catch (...) { return i32{0}; }

        case PropertyType::Int64:
            try { return static_cast<i64>(std::stoll(str)); }
            catch (...) { return i64{0}; }

        case PropertyType::Float:
            try { return static_cast<f32>(std::stof(str)); }
            catch (...) { return f32{0.0f}; }

        case PropertyType::Double:
            try { return std::stod(str); }
            catch (...) { return f64{0.0}; }
...
const char* getTypeName(PropertyType type)
{
    switch (type)
    {
        ...
        case PropertyType::CurveRef: return "CurveRef";
...
bool validate(const PropertyValue& value, const PropertyMeta& meta, std::string* error)
{
    ...
    {
        bool empty = std::visit([](const auto& v) -> bool
        {
            using T = std::decay_t<decltype(v)>;
            if constexpr (std::is_same_v<T, std::nullptr_t>)
                return true;
    ...
    {
        f64 numValue = 0.0;
        bool isNumeric = std::visit([&numValue](const auto& v) -> bool
        {
            using T = std::decay_t<decltype(v)>;
...
{
    return std::visit([&range](const auto& v) -> PropertyValue
    {
        using T = std::decay_t<decltype(v)>;
        ...
        if constexpr (std::is_same_v<T, i32>)
        {
            ...
            if (range.hasMin) clamped = std::max(clamped, range.min);
            if (range.hasMax) clamped = std::min(clamped, range.max);
            if (range.step > 0)
            {
