...
void AudioManager::releaseSource(AudioHandle handle)
{
    m_sources.erase(
        std::remove_if(m_sources.begin(), m_sources.end(),
            [&handle](const auto& s) { return s && s->handle.id == handle.id; }),
        m_sources.end()
    );
}

void AudioManager::fireEvent(AudioEvent::Type type, AudioHandle handle, const std::string& trackId)
{
    if (m_eventCallback)
    {
        AudioEvent event;
...
void AudioManager::updateDucking(f64 deltaTime)
{
    ...
    {
        m_currentDuckLevel = std::min(m_targetDuckLevel,
            m_currentDuckLevel + static_cast<f32>(deltaTime) * speed);
    }
    else
    {
        m_currentDuckLevel = std::max(m_targetDuckLevel,
            m_currentDuckLevel - static_cast<f32>(deltaTime) * speed);
    }
}
