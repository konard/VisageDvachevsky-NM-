...
std::optional<ObjectDescriptor> SceneInspectorAPI::getObject(const std::string& id) const
{
    ...
    switch (layer)
    {
        case LayerType::Background: desc.layer = "Background"; break;
        case LayerType::Characters: desc.layer = "Characters"; break;
        case LayerType::UI: desc.layer = "UI"; break;
        case LayerType::Effects: desc.layer = "Effects"; break;
    }
...
std::vector<PropertyDescriptor> SceneInspectorAPI::getProperties(const std::string& objectId) const
{
    ...
    switch (obj->getType())
    {
        ...
        case SceneObjectType::Character:
        {
            ...
            props.push_back(createPropertyDescriptor("pose", PropertyDescriptor::Type::String,
                                                      character->getPose()));
        ...
        case SceneObjectType::DialogueUI:
        {
            ...
            props.push_back(createPropertyDescriptor("speaker", PropertyDescriptor::Type::String,
                                                      dialogue->getSpeaker()));
            props.push_back(createPropertyDescriptor("text", PropertyDescriptor::Type::String,
                                                      dialogue->getText()));
            props.push_back(createPropertyDescriptor("backgroundTextureId", PropertyDescriptor::Type::Resource,
                                                          ...
                                                          const std::string& propertyName) const
{
    ...
    if (propertyName == "x") return std::to_string(obj->getX());
    if (propertyName == "y") return std::to_string(obj->getY());
    if (propertyName == "scaleX") return std::to_string(obj->getScaleX());
    if (propertyName == "scaleY") return std::to_string(obj->getScaleY());
    if (propertyName == "rotation") return std::to_string(obj->getRotation());
    if (propertyName == "alpha") return std::to_string(obj->getAlpha());
    if (propertyName == "visible") return obj->isVisible() ? "true" : "false";
    if (propertyName == "zOrder") return std::to_string(obj->getZOrder());
...
Result<void> SceneInspectorAPI::scaleObject(const std::string& id, f32 scaleX, f32 scaleY, bool recordUndo)
{
    auto* obj = m_sceneGraph->findObject(id);
    if (!obj)
    {
    ...
    if (recordUndo)
    {
        ...
        auto cmd = std::make_unique<SetPropertyCommand>(this, id, "scaleX", oldScaleX, std::to_string(scaleX));
        cmd->execute();
        m_undoStack.push(std::move(cmd));
        while (!m_redoStack.empty())
        {
...
Result<void> SceneInspectorAPI::rotateObject(const std::string& id, f32 angle, bool recordUndo)
{
    ...
    if (recordUndo)
    {
        ...
        auto cmd = std::make_unique<SetPropertyCommand>(this, id, "rotation", oldAngle, std::to_string(angle));
        cmd->execute();
        m_undoStack.push(std::move(cmd));
        while (!m_redoStack.empty())
        {
...
Result<void> SceneInspectorAPI::setObjectZOrder(const std::string& id, i32 zOrder, bool recordUndo)
{
...
void SceneInspectorAPI::paste(f32 offsetX, f32 offsetY)
{
    ...
    for (const auto& state : m_clipboard)
    {
        ...
        if (obj)
        {
            ...
            switch (state.type)
            {
                case SceneObjectType::Background: layer = LayerType::Background; break;
                case SceneObjectType::Character: layer = LayerType::Characters; break;
                case SceneObjectType::DialogueUI:
                case SceneObjectType::ChoiceUI: layer = LayerType::UI; break;
                case SceneObjectType::EffectOverlay: layer = LayerType::Effects; break;
                default: break;
            }
...
std::vector<PropertyDescriptor> SceneInspectorAPI::getBaseProperties(const SceneObjectBase* obj) const
{
...
std::unique_ptr<SceneObjectBase> SceneInspectorAPI::createObjectOfType(SceneObjectType type,
