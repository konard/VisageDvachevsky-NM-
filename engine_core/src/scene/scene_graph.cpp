...
const char* SceneObjectBase::getTypeName() const
{
    switch (m_type)
    {
        case SceneObjectType::Base: return "Base";
        case SceneObjectType::Background: return "Background";
        case SceneObjectType::Character: return "Character";
        case SceneObjectType::DialogueUI: return "DialogueUI";
        case SceneObjectType::ChoiceUI: return "ChoiceUI";
        case SceneObjectType::EffectOverlay: return "EffectOverlay";
        case SceneObjectType::Sprite: return "Sprite";
        case SceneObjectType::TextLabel: return "TextLabel";
        case SceneObjectType::Panel: return "Panel";
        case SceneObjectType::Custom: return "Custom";
    }
    return "Unknown";
}
...
void SceneObjectBase::animateScale(f32 toScaleX, f32 toScaleY, f32 duration, EaseType easing)
{
    auto tweenX = std::make_unique<FloatTween>(
        &m_transform.scaleX,
        m_transform.scaleX, toScaleX,
        duration, easing
    );
    auto tweenY = std::make_unique<FloatTween>(
        &m_transform.scaleY,
        m_transform.scaleY, toScaleY,
        duration, easing
    );
    tweenX->start();
    tweenY->start();
...
void CharacterObject::loadState(const SceneObjectState& state)
{
    ...
    if (it != state.properties.end()) m_characterId = it->second;

    it = state.properties.find("displayName");
    if (it != state.properties.end()) m_displayName = it->second;

    it = state.properties.find("expression");
    if (it != state.properties.end()) m_expression = it->second;

    it = state.properties.find("pose");
    if (it != state.properties.end()) m_pose = it->second;
...
void DialogueUIObject::loadState(const SceneObjectState& state)
{
    ...
    if (it != state.properties.end()) m_speaker = it->second;

    it = state.properties.find("text");
    if (it != state.properties.end()) m_text = it->second;

    it = state.properties.find("backgroundTextureId");
    if (it != state.properties.end()) m_backgroundTextureId = it->second;
...
void ChoiceUIObject::loadState(const SceneObjectState& state)
{
    ...
    if (countIt != state.properties.end())
    {
        ...
        for (size_t i = 0; i < count; ++i)
        {
            ...
            if (it != state.properties.end()) option.id = it->second;

            it = state.properties.find(prefix + "text");
            if (it != state.properties.end()) option.text = it->second;

            it = state.properties.find(prefix + "enabled");
            if (it != state.properties.end()) option.enabled = (it->second == "true");

            it = state.properties.find(prefix + "visible");
            if (it != state.properties.end()) option.visible = (it->second == "true");
...
Layer& SceneGraph::getLayer(LayerType type)
{
    switch (type)
    {
        case LayerType::Background: return m_backgroundLayer;
        case LayerType::Characters: return m_characterLayer;
        case LayerType::UI: return m_uiLayer;
        case LayerType::Effects: return m_effectLayer;
    }
...
std::unique_ptr<SceneObjectBase> SceneGraph::removeFromLayer(LayerType layer, const std::string& id)
{
                                           ...
                                           CharacterObject::Position position)
{
    ...
    switch (position)
    {
        case CharacterObject::Position::Left: x = 200.0f; break;
        case CharacterObject::Position::Center: x = 640.0f; break;
        case CharacterObject::Position::Right: x = 1080.0f; break;
        case CharacterObject::Position::Custom: break;
    }
...
{
    notifyObservers([&](ISceneObserver* obs) {
        obs->onObjectAdded(objectId, type);
    });
}
...
{
    notifyObservers([&](ISceneObserver* obs) {
        obs->onLayerChanged(objectId, newLayer);
    });
}
