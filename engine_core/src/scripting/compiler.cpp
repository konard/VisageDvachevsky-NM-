...
void Compiler::emit(OpCode op, u32 operand)
{
...
void Compiler::compileShowStmt(const ShowStmt& stmt)
{
    switch (stmt.target)
    {
        case ShowStmt::Target::Background:
        {
            ...
            emit(OpCode::SHOW_BACKGROUND, resIndex);
            break;
        }
        ...
        case ShowStmt::Target::Sprite:
        {
            ...
            emit(OpCode::PUSH_STRING, idIndex);
            ...
            if (stmt.position.has_value())
            {
                ...
                {
                    case Position::Left: posCode = 0; break;
                    case Position::Center: posCode = 1; break;
                    case Position::Right: posCode = 2; break;
                    case Position::Custom: posCode = 3; break;
                }
            }
            emit(OpCode::PUSH_INT, static_cast<u32>(posCode));

            emit(OpCode::SHOW_CHARACTER, idIndex);
            break;
        }
    }
    ...
    if (stmt.transition.has_value())
    {
        ...
        emit(OpCode::PUSH_INT, durInt);
        emit(OpCode::TRANSITION, transIndex);
    }
}
...
{
    u32 idIndex = addString(stmt.identifier);
    emit(OpCode::HIDE_CHARACTER, idIndex);
...
void Compiler::compileSayStmt(const SayStmt& stmt)
{
    ...
    if (stmt.speaker.has_value())
    {
        ...
        emit(OpCode::PUSH_STRING, speakerIndex);
    }
    else
    {
...
void Compiler::compileChoiceStmt(const ChoiceStmt& stmt)
{
    ...
    for (size_t i = 0; i < stmt.options.size(); ++i)
    {
        ...
        emit(OpCode::DUP);
        emit(OpCode::PUSH_INT, static_cast<u32>(i));
        emit(OpCode::EQ);
...
void Compiler::compilePlayStmt(const PlayStmt& stmt)
{
    ...
    {
        emit(OpCode::PLAY_SOUND, resIndex);
    }
    else
    {
        emit(OpCode::PLAY_MUSIC, resIndex);
    }
}
...
void Compiler::compileTransitionStmt(const TransitionStmt& stmt)
{
    ...
    std::memcpy(&durInt, &stmt.duration, sizeof(f32));
    emit(OpCode::PUSH_INT, durInt);
    emit(OpCode::TRANSITION, typeIndex);
}
...
void Compiler::compileLiteral(const LiteralExpr& expr)
{
    std::visit([this](const auto& val)
    {
        ...
        {
            emit(OpCode::PUSH_INT, static_cast<u32>(val));
        }
        else if constexpr (std::is_same_v<T, f32>)
        {
            u32 intRep = 0;
            std::memcpy(&intRep, &val, sizeof(f32));
            emit(OpCode::PUSH_FLOAT, intRep);
        }
        else if constexpr (std::is_same_v<T, bool>)
        {
            emit(OpCode::PUSH_BOOL, val ? 1 : 0);
        }
...
void Compiler::compileIdentifier(const IdentifierExpr& expr)
{
    ...
    emit(OpCode::LOAD_GLOBAL, nameIndex);
}
...
void Compiler::compileBinary(const BinaryExpr& expr)
{
    ...
    switch (expr.op)
    {
        ...
        case TokenType::NotEqual:       emit(OpCode::NE); break;
        case TokenType::Less:           emit(OpCode::LT); break;
        case TokenType::LessEqual:      emit(OpCode::LE); break;
        case TokenType::Greater:        emit(OpCode::GT); break;
        case TokenType::GreaterEqual:   emit(OpCode::GE); break;
        default:
