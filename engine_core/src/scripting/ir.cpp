...
IRNode::IRNode(NodeId id, IRNodeType type)
    : m_id(id)
    , m_type(type)
{
}
...
const char* IRNode::getTypeName() const
{
    switch (m_type)
    {
        case IRNodeType::SceneStart: return "SceneStart";
        case IRNodeType::SceneEnd: return "SceneEnd";
        case IRNodeType::Comment: return "Comment";
        case IRNodeType::Sequence: return "Sequence";
        case IRNodeType::Branch: return "Branch";
        case IRNodeType::Switch: return "Switch";
        case IRNodeType::Loop: return "Loop";
        case IRNodeType::Goto: return "Goto";
        case IRNodeType::Label: return "Label";
        case IRNodeType::ShowCharacter: return "ShowCharacter";
        case IRNodeType::HideCharacter: return "HideCharacter";
        case IRNodeType::ShowBackground: return "ShowBackground";
        case IRNodeType::Dialogue: return "Dialogue";
        case IRNodeType::Choice: return "Choice";
        case IRNodeType::ChoiceOption: return "ChoiceOption";
        case IRNodeType::PlayMusic: return "PlayMusic";
        case IRNodeType::StopMusic: return "StopMusic";
        case IRNodeType::PlaySound: return "PlaySound";
        case IRNodeType::Transition: return "Transition";
        case IRNodeType::Wait: return "Wait";
        case IRNodeType::SetVariable: return "SetVariable";
        case IRNodeType::GetVariable: return "GetVariable";
        case IRNodeType::Expression: return "Expression";
        case IRNodeType::FunctionCall: return "FunctionCall";
        case IRNodeType::Custom: return "Custom";
    }
    return "Unknown";
}
...
std::string IRNode::getStringProperty(const std::string& name, const std::string& defaultValue) const
{
        ...
        while (!queue.empty())
        {
            ...
            for (const auto& conn : getConnectionsFrom(id))
            {
                if (conn.source.portName.find("exec") != std::string::npos ||
                    conn.source.portName == "true" ||
                    conn.source.portName == "false")
                {
                    queue.push(conn.target.nodeId);
                }
            }
        ...
        for (const auto& port : node->getInputPorts())
        {
            if (port.required && !port.isExecution)
            {
                ...
                if (!found)
                {
                    ...
                    {
                        errors.push_back("Node " + std::to_string(id) + " missing required input: " + port.name);
                    }
                }
            }
        }
...
void IRGraph::addCharacter(const std::string& id, const std::string& name, const std::string& color)
{
...
NodeId ASTToIRConverter::convertStatement(const Statement& stmt, NodeId prevNode)
{
    ...
    return std::visit([this, prevNode, &stmt](const auto& stmtData) -> NodeId {
        ...
        if constexpr (std::is_same_v<T, ShowStmt>)
        {
            IRNodeType nodeType = (stmtData.target == ShowStmt::Target::Background) ?
                IRNodeType::ShowBackground : IRNodeType::ShowCharacter;
        ...
        else if constexpr (std::is_same_v<T, PlayStmt>)
        {
            IRNodeType nodeType = (stmtData.type == PlayStmt::MediaType::Music) ?
                IRNodeType::PlayMusic : IRNodeType::PlaySound;
            NodeId nodeId = createNodeAndConnect(nodeType, prevNode);
            auto* node = m_graph->getNode(nodeId);
...
std::unique_ptr<Statement> IRToASTConverter::convertNode(const IRNode* node, const IRGraph& /*graph*/)
{
...
std::unique_ptr<Expression> IRToASTConverter::convertToExpression(const IRNode* /*node*/, const IRGraph& /*graph*/)
{
    // Stub implementation
    return nullptr;
}
...
void ASTToTextGenerator::generateStatement(const Statement& stmt, int /*indentLvl*/)
{
    ...
    std::visit([this](const auto& stmtData) {
        using T = std::decay_t<decltype(stmtData)>;
...
{
    std::visit([this](const auto& exprData) {
...
Result<std::unique_ptr<VisualGraph>> RoundTripConverter::textToVisualGraph(const std::string& nmScript)
{
...
std::vector<std::string> RoundTripConverter::validateConversion(const std::string& /*original*/,
