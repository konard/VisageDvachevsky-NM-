...
{
    m_vm.registerCallback(OpCode::SHOW_BACKGROUND, [this](const auto& args) {
        onShowBackground(args);
    });

    m_vm.registerCallback(OpCode::SHOW_CHARACTER, [this](const auto& args) {
        onShowCharacter(args);
    });

    m_vm.registerCallback(OpCode::HIDE_CHARACTER, [this](const auto& args) {
        onHideCharacter(args);
    });

    m_vm.registerCallback(OpCode::SAY, [this](const auto& args) {
        onSay(args);
    });

    m_vm.registerCallback(OpCode::CHOICE, [this](const auto& args) {
        onChoice(args);
    });

    m_vm.registerCallback(OpCode::GOTO_SCENE, [this](const auto& args) {
        onGotoScene(args);
    });

    m_vm.registerCallback(OpCode::WAIT, [this](const auto& args) {
        onWait(args);
    });

    m_vm.registerCallback(OpCode::PLAY_SOUND, [this](const auto& args) {
        onPlaySound(args);
    });

    m_vm.registerCallback(OpCode::PLAY_MUSIC, [this](const auto& args) {
        onPlayMusic(args);
    });

    m_vm.registerCallback(OpCode::STOP_MUSIC, [this](const auto& args) {
        onStopMusic(args);
    });

    m_vm.registerCallback(OpCode::TRANSITION, [this](const auto& args) {
        onTransition(args);
    });
}
...
Scene::CharacterPosition ScriptRuntime::parsePosition(i32 posCode)
{
    switch (posCode)
    {
        case 0: return Scene::CharacterPosition::Left;
        case 1: return Scene::CharacterPosition::Center;
        case 2: return Scene::CharacterPosition::Right;
        default: return Scene::CharacterPosition::Custom;
    }
}
