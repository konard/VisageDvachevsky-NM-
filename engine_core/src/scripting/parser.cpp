...
{
    if (match(TokenType::Show)) return parseShowStmt();
    if (match(TokenType::Hide)) return parseHideStmt();
    if (match(TokenType::Say)) return parseSayStmt();
    if (match(TokenType::Choice)) return parseChoiceStmt();
    if (match(TokenType::If)) return parseIfStmt();
    if (match(TokenType::Goto)) return parseGotoStmt();
    if (match(TokenType::Wait)) return parseWaitStmt();
    if (match(TokenType::Play)) return parsePlayStmt();
    if (match(TokenType::Stop)) return parseStopStmt();
    if (match(TokenType::Set)) return parseSetStmt();
    if (match(TokenType::Transition)) return parseTransitionStmt();
...
StmtPtr Parser::parseShowStmt()
{
    ...
    if (match(TokenType::Transition))
    {
        ...
        if (check(TokenType::Float) || check(TokenType::Integer))
        {
            ...
            stmt.duration = dur.type == TokenType::Float ? dur.floatValue : static_cast<f32>(dur.intValue);
        }
    }
...
StmtPtr Parser::parseTransitionStmt()
{
    ...
    // Transition type can be a keyword (fade) or identifier (dissolve, slide, etc.)
