...
{
    if (m_points.empty()) return {0.0f, 0.0f};
    ...
    if (index >= m_points.size() - 1) index = m_points.size() - 2;
    ...
    // Apply easing
    localT = scene::applyEasing(localT, p1.easing);

    return {
        p0.position.x + (p1.position.x - p0.position.x) * localT,
...
f32 CameraPath::evaluateZoom(f32 t) const
{
    ...
    if (m_points.size() == 1) return m_points[0].zoom;
...
f32 CameraPath::evaluateRotation(f32 t) const
{
    ...
    if (m_points.size() == 1) return m_points[0].rotation;
...
void Camera2D::moveTo(f32 x, f32 y, f32 duration, scene::EasingType easing)
{
...
void Camera2D::zoomTo(f32 zoom, f32 duration, scene::EasingType easing)
{
    m_startZoom = m_zoom;
    m_targetZoom = zoom;
    m_transitionDuration = duration;
...
void Camera2D::zoomAt(f32 zoom, f32 x, f32 y, f32 duration, scene::EasingType easing)
{
...
void Camera2D::rotateTo(f32 angle, f32 duration, scene::EasingType easing)
{
...
void Camera2D::transitionTo(f32 x, f32 y, f32 zoom, f32 rotation, f32 duration, scene::EasingType easing)
{
    m_startPosition = m_position;
    m_targetPosition = {x, y};
    m_startZoom = m_zoom;
...
{
    return {
        m_position.x * (1.0f - depth),
        m_position.y * (1.0f - depth)
    };
}
...
void Camera2D::updateTransitions(f64 deltaTime)
{
    ...
    f32 easedT = scene::applyEasing(t, m_transitionEasing);
...
void Camera2D::updateTarget(f64 deltaTime)
{
    ...
    // Target following implementation would require access to scene objects
...
{
    if (!m_shakeActive) return {0.0f, 0.0f};
...
{
    if (name == m_activeCameraName) return; // Can't remove active camera
...
void CameraManager::transitionToCamera(const std::string& cameraName, f32 duration, scene::EasingType easing)
{
    if (m_cameras.find(cameraName) == m_cameras.end()) return;
    if (cameraName == m_activeCameraName) return;
...
Transform2D CameraManager::getBlendedTransform() const
{
    ...
    t = scene::applyEasing(t, m_cameraTransitionEasing);
...
{
    camera.moveTo(targetX, targetY, 3.0f, scene::EasingType::EaseInOut);
}

void applyCinematicPan(Camera2D& camera, f32 targetX, f32 targetY)
{
    camera.moveTo(targetX, targetY, 2.0f, scene::EasingType::EaseOutCubic);
}

void applyDramaticZoom(Camera2D& camera, f32 targetZoom)
{
    camera.zoomTo(targetZoom, 1.5f, scene::EasingType::EaseOutQuad);
}

void applyCloseUp(Camera2D& camera, f32 x, f32 y)
{
    camera.transitionTo(x, y, 1.5f, 0.0f, 1.0f, scene::EasingType::EaseOutCubic);
}

void applyEstablishingShot(Camera2D& camera)
{
    camera.transitionTo(0.0f, 0.0f, 0.8f, 0.0f, 2.0f, scene::EasingType::EaseInOut);
}
