...
TextLayout TextLayoutEngine::layout(const std::string& text) const
{
    ...
    f32 lineWidth = 0.0f;
    f32 lineHeight = m_defaultStyle.size * m_lineHeight;
    i32 charCount = 0;
...
std::pair<f32, f32> TextLayoutEngine::getCharacterPosition(const TextLayout& layout, i32 targetIndex) const
{
    f32 currentY = 0.0f;
    i32 charIndex = 0;
    ...
    for (const auto& line : layout.lines)
    {
        ...
        for (const auto& segment : line.segments)
        {
            ...
            for (char c : segment.text)
            {
                ...
                currentX += measureChar(c, segment.style);
                ++charIndex;
            }
        }
