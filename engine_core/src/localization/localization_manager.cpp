...
StringTable::StringTable(const LocaleId& locale)
    : m_locale(locale)
{
}
...
std::optional<std::string> StringTable::getPluralString(const std::string& id, i64 count) const
{
    ...
    if (count == 0) category = PluralCategory::Zero;
    else if (count == 1) category = PluralCategory::One;
...
Result<void> LocalizationManager::loadStringsFromMemory(const LocaleId& locale, const std::string& data,
                                                         LocalizationFormat format)
{
    switch (format)
    {
...
std::string LocalizationManager::get(const std::string& id,
...
std::string LocalizationManager::getPlural(const std::string& id, i64 count,
                                            const std::unordered_map<std::string, std::string>& variables) const
{
...
void LocalizationManager::setString(const LocaleId& locale, const std::string& id, const std::string& value)
{
...
Result<void> LocalizationManager::exportMissingStrings(const LocaleId& locale, const std::string& filePath,
                                                        LocalizationFormat format) const
{
...
std::string LocalizationManager::interpolate(const std::string& text,
                                              const std::unordered_map<std::string, std::string>& variables) const
{
...
Result<void> LocalizationManager::exportCSV(const StringTable& table, const std::string& path) const
{
...
Result<void> LocalizationManager::exportJSON(const StringTable& table, const std::string& path) const
{
    std::ofstream file(path);
    if (!file.is_open())
    {
...
Result<void> LocalizationManager::exportPO(const StringTable& table, const std::string& path) const
{
    std::ofstream file(path);
    if (!file.is_open())
    {
