...
Rect Widget::measure(f32 availableWidth, f32 availableHeight)
{
    ...
    f32 height = m_constraints.preferredHeight > 0 ? m_constraints.preferredHeight :
...
void HBox::layoutChildren()
{
    ...
    // Account for spacing
    f32 spacing = m_spacing * static_cast<f32>(
        std::count_if(m_children.begin(), m_children.end(),
            [](const auto& c) { return c->isVisible(); }) - 1);
...
void Slider::render(renderer::IRenderer& renderer)
{
    ...
    renderer::Color thumbColor = m_dragging || m_hovered ? m_style.hoverColor : m_style.foregroundColor;
...
ScrollPanel::ScrollPanel(const std::string& id)
    : Container(id)
{
}
...
void ScrollPanel::render(renderer::IRenderer& renderer)
{
    ...
    if (m_verticalScroll && m_contentHeight > m_bounds.height)
    {
        ...
        renderer::Rect scrollbarRect{m_bounds.x + m_bounds.width - scrollbarWidth, m_bounds.y + scrollbarY, scrollbarWidth, scrollbarHeight};
...
void UIManager::handleKeyDown(i32 keyCode)
{
    ...
    if (keyCode == 16) m_shiftDown = true;
    if (keyCode == 17) m_ctrlDown = true;
    if (keyCode == 18) m_altDown = true;
...
{
    if (keyCode == 16) m_shiftDown = false;
    if (keyCode == 17) m_ctrlDown = false;
    if (keyCode == 18) m_altDown = false;
